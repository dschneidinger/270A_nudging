...
# the version where the nn predicts f_t+1 from phi_t
# there should be another version where the nn predicts thee residual f_t+1-f_t from phi_t
def da_step_nudging1d1v(
    model,
    states_current,
    # states_obs_next,
    density_nn_next,
    ...
    # replace with noisy, sparse measurements
    # look at amit and david's code on github
    ...
    for _ in range(nudge_iters):
        ...
        residual = density_pred - density_nn_next
        
        print('residual shape:', residual.shape)
        print('kx_pred shape:', kx_pred.shape)
...
def run_data_assimilation(args):
    ###################################
    # BEGIN NN INITIALIZATION PORTION #
    ###################################
    config = {
    ...
    # for t in tqdm(range(n_steps + 1), desc='Building true phase density and observations'):
    ...
    # THIS MIGHT NEED CHANGE TO MATCH THE INPUT DATA OF THE NN
    ...
    # NN setup
    print('Loading the model for training...')
    ...
    print('Model loading done. Begin training!')
    # training loop
    for t in tqdm(range(n_steps), desc='Training the model'):
    # for epoch in range(num_epochs):
        ...
        for inputs, targets in train_loader:
            ...
            print(outputs.shape)
            states_est, err_obs, density_est, rho_x_now, electric_now = da_step_nudging1d1v(
                model=model,
                states_current=states_est,
                # states_obs_next=states_obs[t + 1],
                density_nn_next=outputs[0].cpu().detach().numpy(),
            ...
            # print(density_est.shape)

            # density_est_flat = density_est.view(density_est.size(0), -1)
            density_est_flat = density_est.flatten()
            density_est_flat = torch.from_numpy(density_est_flat)
            # loss = criterion(outputs_flat, targets)
            targets_2d = targets.view(-1, model.n_x_grid, model.n_v_grid)
            print('density_est_flat shape', density_est_flat.shape)
            print('targets shape', targets.shape)
            # outputs_flat = outputs.view(outputs.size(0), -1)
            # density_est_flat = torch.from_numpy(density_est)
            # loss = criterion(outputs_flat, targets)
            print(density_est_flat.shape)
            print(targets[0].shape)
            loss = criterion(density_est_flat, targets[0])
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for inputs, targets in val_loader:
                ...
                states_est, err_obs, density_est, rho_x_now, electric_now = da_step_nudging1d1v(
                    states_current=states_est,
                    # states_obs_next=states_obs[t + 1],
                    density_nn_next=outputs.cpu().detach().numpy(),
                    # mask_next=mask_missing[t + 1],
                    x_mesh=x_mesh,
                    v_mesh=v_mesh,
                    dx=dx,
                ...
                print('density_est_flat shape', density_est_flat.shape)
                print('targets shape', targets.shape)
                loss = criterion(density_est_flat, targets)
                # loss = criterion(outputs_flat, targets)
        ...
        # NEW: replace with NN
        # input: states_pred, x_mesh, v_mesh, length, hx_bw, hv_bw
        # output: control_x, control_v
    ...
    save_path = os.path.join(args.outdir, f"phase_da_nnnudge_1d1v_seed{args.seed}.npz")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-3,
        help="Learning rate for the NN model."
    )
