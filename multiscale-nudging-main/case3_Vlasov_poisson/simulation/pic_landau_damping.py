#!/usr/bin/env python3
"""1D-1V electrostatic PIC for Landau damping with DA-ready outputs."""
...
def quiet_start_velocities(
    num_particles: int, vmax: float, rng: np.random.Generator
) -> np.ndarray:
    """Deterministic truncated-Maxwellian loading via inverse CDF."""
...
def random_velocities(
    ...
    """Sample a truncated Maxwellian N(0, 1) on [-vmax, vmax]."""
    accepted = []
    remaining = num_particles

    while remaining > 0:
        batch = max(2 * remaining, 1024)
        v_trial = rng.normal(loc=0.0, scale=1.0, size=batch)
...
def load_particles(
    ...
    if cfg.random_load:
        ...
        vp = random_velocities(cfg.num_particles, cfg.vmax, rng)
    else:
        xp = quiet_start_positions(
            cfg.num_particles, cfg.length, cfg.epsilon, cfg.mode
        )
        vp = quiet_start_velocities(cfg.num_particles, cfg.vmax, rng)
    return xp, vp
...
def write_summary(
    ...
    lines = [
        "1D-1V electrostatic PIC Landau damping",
...
def maybe_make_plot(output_dir: Path, times: np.ndarray, e_mode: np.ndarray) -> None:
    ...
    ax.set_title("PIC Landau damping")
